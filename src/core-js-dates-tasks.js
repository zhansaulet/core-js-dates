...
function isDateInPeriod(date, period) {
  ...
  const dateGivenDaysCount = dateGiven.getTime() / (1000 * 60 * 60 * 24);
  const startDaysCount = start.getTime() / (1000 * 60 * 60 * 24);
  const endDaysCount = end.getTime() / (1000 * 60 * 60 * 24);

  if (
    dateGivenDaysCount >= startDaysCount &&
    dateGivenDaysCount <= endDaysCount
  ) {
    return true;
  }
...
function formatDate(date) {
  ...
  return dateFormat.toLocaleString('en-US', { timeZone: 'UTC' });
}
...
function getWeekNumberByDate(date) {
  ...
  const diff = (date - startDate) / milliseconds;

  return Math.ceil((diff + 1 + ((startDate.getDay() + 6) % 7)) / 7);
}
...
function getQuarter(date) {
  ...
  if (month >= 4 && month <= 6) result = 2;
  if (month >= 7 && month <= 9) result = 3;
  if (month >= 10 && month <= 12) result = 4;
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  while (start <= end) {
    for (let i = 1; i <= countWorkDays; i += 1) {
      ...
      tempDate = new Date(start).setDate(new Date(start).getDate() + 1);
      start = new Date(tempDate);
    }
    start = new Date(tempDate).setDate(
      new Date(tempDate).getDate() + countOffDays
    );
  }
  return scheduleArr;
}
